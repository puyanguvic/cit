...
\Description{Two line charts (tiny and small models) showing accuracy versus token budget for five tokenizers (BPE, Bytes, CIT, Unigram, WordPiece). Solid lines denote test accuracy and dashed lines denote drift accuracy; CIT achieves the highest accuracy across budgets.}
...
\Description{Three-panel trade-off plot on Adult: accuracy versus average tokens per sample (rate), accuracy versus P95 latency in milliseconds (log-scaled x-axis), and accuracy versus parameter count. Points are colored by tokenizer (BPE, Bytes, CIT, Unigram, WordPiece) with circles for mini and squares for small models; connectors show the change from mini to small, and arrows indicate the preferred direction.}
\label{fig:e3_pareto}
\end{figure*}
...
Tokenization is not only a compression heuristic: under a strict drop-in regime it is a \emph{hard interface} that determines which prefix distinctions are even observable to the model.
...
While we treat the contract as a specified artifact, many structured deployments already expose schemas, grammars, and field validators that can be reused to derive role boundaries and value types.
An important direction is contract-assistance tooling: automatically proposing candidate typed spans that dominate rate tails or that are brittle under benign re-serializations, then validating them through rate--distortion measurements.
...
We include additional controls, proofs, and extended context in the supplementary appendix to support this workflow.
