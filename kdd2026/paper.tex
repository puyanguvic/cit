...
Taken together, the results establish a consistent causal chain.
Off-the-shelf tokenizers exhibit substantial interface mismatch on structured inputs.
In controlled settings, this mismatch leads to prefix semantic collapse, which CIT prevents through role- and type-aware constraints.
Under matched compute budgets, these interface improvements translate into higher robustness and better sample efficiency.
End-to-end evaluations show that CIT improves accuracy--latency trade-offs, and additional datasets confirm that the observed trends generalize beyond a single domain.
