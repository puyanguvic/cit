...
In contrast, CIT prevents collapse by enforcing typed-span integrity and role-aware boundaries, achieving perfect in-distribution accuracy and strong drift robustness at a substantially lower rate.
The lossless byte-level interface preserves raw information but is prohibitively high-rate under a strict length cap, leading to truncation and poor downstream accuracy under compute-matched training.
...
This shifts the accuracy--length frontier and yields large robustness gains under drift: CIT maintains its performance on the drift slice while standard subword baselines can suffer large drops.
...
CIT reduces average and tail sequence lengths relative to standard subword baselines (about 61 tokens on average vs.\ about 106 for BPE/WordPiece at the same vocabulary budget), yielding consistent latency reductions at comparable accuracy.
The byte-level baseline provides a complementary operating point with high fidelity but substantially higher latency due to its rate.
