...
(Vocabulary-budget sweeps and additional details are deferred to the supplementary appendix.)
...
The main takeaway is that tokenization is not only a compression heuristic: under a strict drop-in regime it is a \emph{hard interface} that determines which prefix distinctions are even observable to the model.
For structured streams, interface errors are often causal and irreversible (prefix semantic collapse), so improving the interface can be more effective than increasing model size or training compute under the same constraints.

\paragraph{When CIT helps (and why).}
CIT is most appropriate when (i) semantics is role- and boundary-dependent (e.g., key/value and delimiter structure), (ii) high-cardinality values are frequent and span integrity matters, and (iii) deployment budgets make truncation and length tails a first-order concern.
In these settings, frequency-driven subword merges can trade away role cues for compression, while byte-level interfaces preserve everything but waste rate.
CIT targets the middle ground: it enforces role and typed-value constraints to prevent collapse, then uses deployment-aligned gain to recover efficiency.
...
Finally, our directed semantic distortion is an evaluation tool; while it is designed to reflect interface-induced loss, any practical surrogate depends on the chosen teacher/student capacity and the data distribution.

\paragraph{Practical guidance.}
In deployments, the highest-leverage step is to define the serialization and contract as a versioned artifact alongside the model, and to audit it under realistic drift operators (e.g., benign re-serializations) before vocabulary induction.
When contract design is unclear, a conservative workflow is to start with minimal hard constraints (obvious boundaries and typed spans), measure rate--distortion operating points, and iterate only where the interface demonstrably collapses semantics.
We include additional controls, proofs, and vocabulary-budget sweeps in the supplementary appendix to support this workflow.
