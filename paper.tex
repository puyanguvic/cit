...
We evaluate CIT with three main-paper experiments that trace a causal chain from interface behavior to learning dynamics to system-level outcomes:
\textbf{E1} asks whether prefix-emitting tokenization induces semantic collapse and whether CIT prevents it;
\textbf{E2} asks whether preventing collapse improves end-to-end learning and drift robustness under a fixed encoder-token budget;
...
Across experiments, we keep the serialization schema, model family, and optimizer settings fixed, varying only the tokenization interface.
...
training dynamics (validation accuracy as a function of seen encoder tokens, including tokens-to-threshold summaries),
...
This task exhibits a clear interface failure mode: standard subword tokenizers can induce prefix semantic collapse, driving accuracy toward chance despite compute-matched training.
In contrast, CIT prevents collapse by enforcing role/type integrity at the interface, achieving high accuracy and substantially lower surrogate distortion.
The byte-level baseline is lossless but extremely high-rate; under a strict length cap and compute-matched training, it suffers severe truncation and low downstream accuracy.
CIT attains high semantic fidelity at substantially lower interface cost.
Under semantics-preserving drift, CIT remains stable while baselines degrade, consistent with collapse being an interface property rather than a modeling artifact.
...
\bottomrule
\end{tabular}
\caption{E1 (synthetic) summary (seed0).}
\label{tab:e1_synth}
\end{table}

This experiment highlights a rate--distortion trade-off under prefix-emitting interfaces: compared to budget-matched subword tokenizers, CIT attains substantially lower $\widehat{\Delta}(\tau)$ and correspondingly better drift robustness, while remaining far more token-efficient than the lossless bytes interface.
...
Under token-fair training, CIT achieves substantially shorter sequences than standard subword baselines, which increases effective sample throughput and stabilizes optimization under the same token budget.
Compared to the lossless byte-level baseline, CIT achieves substantially higher robustness at a far lower interface cost by enforcing interface constraints rather than relying on lossless tokenization.
In final test accuracy, CIT is competitive or better under compute-matched budgets, with the largest gains appearing in drift robustness.
...
\bottomrule
\end{tabular}
\caption{E2 (CSIC 2010 HTTP) token-fair summary (seed0).}
...
Beyond final accuracy, learning curves (validation accuracy vs.\ seen encoder tokens) show that CIT reaches strong operating points in fewer tokens and with fewer optimization steps under token-fair budgets (Figure~\ref{fig:e2_csic_token_fair_learning_curves_tiny}).
This provides an end-to-end notion of \emph{convergence under interface constraints} that complements the interface-only distortion diagnostics.
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{Figs/e2_csic_token_fair_learning_curves_tiny.pdf}
\caption{E2 (CSIC 2010 HTTP) token-fair learning curves (Tiny, 10M tokens, $L_{\max}{=}512$): validation accuracy as a function of seen encoder tokens.}
\label{fig:e2_csic_token_fair_learning_curves_tiny}
\end{figure}
...
CIT reduces average and tail sequence lengths relative to standard subword baselines, yielding consistent latency reductions at comparable accuracy.
The byte-level baseline provides a complementary operating point with high fidelity but higher latency due to its rate.
Overall, CIT improves operating points on the accuracy--latency and accuracy--parameter frontiers by simultaneously reducing interface cost and preserving task-relevant prefix information (lower surrogate distortion), enabling smaller models to match the accuracy of larger baselines in some regimes.
...
\bottomrule
\end{tabular}
\caption{E3 (Pareto slice) summary (seed0).}
\label{tab:e3_pareto}
\end{table}
