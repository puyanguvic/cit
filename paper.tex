...
In a scan of six widely used pretrained tokenizers on 5{,}000 CSIC 2010 HTTP requests with $L_{\max}{=}512$, average sequence lengths range from 359--468 tokens and 7--29\% of examples exceed the limit (29.4\% for BERT WordPiece; 7.3\% for a GPT-3.5/GPT-4-family tokenizer; Appendix Table~\ref{tab:appendix_e0_tokenizer_playground}).
...
We evaluate CIT in three experiments designed to make the interface effects measurable under controlled, deployment-relevant constraints.
...
(Additional public tabular benchmarks, vocabulary-budget sweeps, and scans of popular pretrained tokenizers are deferred to the appendix.)
...
\emph{Off-the-shelf pretrained tokenizers} (e.g., popular BERT/GPT-family tokenizers) are not budget-matched and are trained on different distributions; we therefore report them as a tokenizer-only interface scan in the appendix rather than mixing them into compute-matched end-to-end training.
...
E2 trains end-to-end Tiny/Small encoders on CSIC under a 10M-token budget ($L_{\max}{=}512$); a step-fair control with matched optimizer steps is provided in the appendix.
...
Table~\ref{tab:e2_csic_token_fair} summarizes final accuracy, drift accuracy, and token-length statistics under token-fair training, and Figure~\ref{fig:e2_csic_token_fair_frontier_acc_vs_len} shows the resulting accuracy--length frontier.
\input{results/paper/paper_artifacts/main/tables/e2_csic_token_fair.tex}

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{Figs/e2_csic_token_fair_frontier_acc_vs_len.pdf}
\caption{E2 (CSIC 2010 HTTP) token-fair frontier (10M tokens, $L_{\max}{=}512$): accuracy vs.\ average sequence length. Filled markers show test accuracy and hollow markers show drift accuracy under semantics-preserving re-serialization; vertical segments visualize the robustness gap. Colors denote tokenizers and marker shapes denote model size.}
\label{fig:e2_csic_token_fair_frontier_acc_vs_len}
\end{figure}
...
A step-fair control (appendix) preserves CIT's drift robustness while attenuating accuracy differences, consistent with robustness arising from interface correctness and accuracy gains under token-fair training partially reflecting improved token efficiency.
...
Additional tokenizer scans, step-fair controls, vocabulary sweeps, and public tabular benchmarks are provided in the appendix.
